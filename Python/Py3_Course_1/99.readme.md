...
This is achieved by listing out all the parameters  and separating them with commas. Order matters in pcalling the function, else you need to specify the parameters. For example: calling `def subtract_bc(a,b,c)` as `subtract_bc(45,62,41)` or `subtract_bc(b=45,a=62,c=41)`.
...
This may be used to obtain a desired portion of data or elements from a main source. This can be done by calling the object, followed by square brackets, and then inserting the index of the first element, followed by the index of the element after the last desired element. For Example:

 The index of an element in a list: `Object_Name.iNdex("Element")`
...
Value may then be accessed by its key as opposed to an index.
...
See below, for another syntax used in creating dictionaries. The **.get()** method can be used to obtain an object in a dictionary.
